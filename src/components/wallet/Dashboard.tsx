...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate, isSuperAdmin = false }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Balance Card */}
      <div className="relative overflow-hidden bg-gradient-to-br from-pink-600 via-pink-700 to-fuchsia-800 rounded-2xl p-6 lg:p-8 shadow-2xl shadow-pink-900/30">
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/3" />
        <div className="absolute bottom-0 left-0 w-40 h-40 bg-white/5 rounded-full translate-y-1/2 -translate-x-1/4" />
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Wallet className="w-5 h-5 text-pink-200/70" />
              <span className="text-pink-200/70 text-sm font-medium">Available Balance</span>
            </div>
            <button
          ...
          <div className="mb-6">
            ...
            <p className="text-pink-200/50 text-sm mt-1">Digital Token Balance</p>
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-rose-400/20 hover:bg-rose-400/30 text-rose-100 px-5 py-2.5 rounded-xl text-sm font-medium transition-all backdrop-blur-sm"
            >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            <div className="p-1.5 rounded-lg bg-pink-400/10">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { label: 'Buy Tokens', icon: CreditCard, color: 'from-pink-500 to-pink-600', view: 'purchase' },
            { label: 'Pay with OTP', icon: Zap, color: 'from-rose-500 to-rose-500', view: 'otp' },
            { label: 'History', icon: Clock, color: 'from-purple-500 to-fuchsia-500', view: 'history' },
              ...
              {recentTransactions.map((tx) => (
                <div key={tx.id} className="flex items-center gap-3 p-4 hover:bg-white/5 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {tx.token_key && (
                      <p className="text-[10px] font-mono text-rose-300/60 truncate mt-0.5">Key: {tx.token_key}</p>
                    )}
                  </div>
                  <div className="text-right">
